  ...
  next)
    ...
    case $CURRENT in
      2|3|4|5|7)
        ...
        ;;
      6)
        # Stage 6 to 7 transition: Quality Gates Check
        TASK_ID=$(jq -r '.task_id' "$WORKFLOW_FILE")
        export TASK_ID
        if [ -f "$PROJ_ROOT/scripts/check-quality-gates.sh" ]; then
          echo "{\"status\": \"Checking quality gates...\"}" >&2
          if ! bash "$PROJ_ROOT/scripts/check-quality-gates.sh"; then
            echo "{\"error\": \"Quality gates check failed. Please refer to docs/QUALITY_REPORT.md for details.\"}"
            exit 1
          fi
        else
          echo "{\"warning\": \"scripts/check-quality-gates.sh not found. Skipping gates check.\"}" >&2
        fi
        ;;
      8)
