...
function getGitLog() {
    try {
        ...
        // We limit to the last 100 commits to avoid huge logs and potential performance issues
        return execSync('git log -n 100 --pretty=format:"%h|%ad|%s" --date=short --grep="tdd:"').toString();
    } catch (error) {
        return '';
    }
}
...
function generateTableContent(log) {
    ...
    lines.forEach(line => {
        ...
        if (subject.toLowerCase().startsWith('tdd:red')) {
            status = '🔴 RED';
            description = subject.replace(/tdd:red/i, '').trim();
        } else if (subject.toLowerCase().startsWith('tdd:green')) {
            status = '🟢 GREEN';
            description = subject.replace(/tdd:green/i, '').trim();
        } else if (subject.toLowerCase().startsWith('tdd:refactor')) {
            status = '🔵 REFACTOR';
            description = subject.replace(/tdd:refactor/i, '').trim();
        }
...
function generateTddLog(verifyOnly = false) {
    ...
    if (verifyOnly) {
        ...
        // 1. Signature Check (Strict: Must be the last line and unique)
        // Trim only trailing whitespace for initial check
        const lines = content.trimEnd().split('\n');
        const lastLine = lines[lines.length - 1].trim();
        const signatureMatch = lastLine.match(/^<!-- SIGNATURE: ([a-f0-9]+) -->$/);
        
        if (!signatureMatch) {
            console.error('Error: TDD_LOG.md must end with a valid signature line.');
            process.exit(1);
        }

        const originalSignature = signatureMatch[1];
        
        const allSignatures = content.match(/<!-- SIGNATURE: [a-f0-9]+ -->/g);
        if (allSignatures && allSignatures.length > 1) {
            console.error('Error: TDD_LOG.md contains multiple signature lines.');
            process.exit(1);
        }

        // Check for any content after the signature line in the original file
        const signatureLine = `<!-- SIGNATURE: ${originalSignature} -->`;
        if (!content.endsWith(signatureLine + '\n') && !content.endsWith(signatureLine)) {
             console.error('Error: TDD_LOG.md must end exactly with the signature line.');
             process.exit(1);
        }
        
        // Ensure no multiple signatures (already checked by allSignatures.length > 1)

        // Extract content without the last signature line
        const contentWithoutSignature = content.trim().split('\n').slice(0, -1).join('\n').trim();
        ...
        if (log) {
            ...
            const invalid = verifyCommits(logLines);
            if (invalid.length > 0) {
            ...
            // Check the table content directly to ensure all phases are represented in the current log
            const hasRed = contentWithoutSignature.includes('🔴 RED');
            const hasGreen = contentWithoutSignature.includes('🟢 GREEN');
            const hasRefactor = contentWithoutSignature.includes('🔵 REFACTOR');
