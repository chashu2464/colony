...
const log = new Logger('RateLimitManager');

interface RateLimitEvents {
    'quota_warning': { model: SupportedCLI; remaining: number; total: number };
    'quota_exhausted': { model: SupportedCLI };
    'quota_reset': { model: SupportedCLI };
}

// Default quotas (conservative estimates, can be overridden via config)
const DEFAULT_QUOTAS: Record<SupportedCLI, Omit<ModelQuota, 'currentUsage' | 'windowStartedAt' | 'dailyStartedAt'>> = {
    claude: {
        model: 'claude',
...
export class RateLimitManager {
    private quotas = new Map<SupportedCLI, ModelQuota>();
    readonly events = new EventBus<RateLimitEvents>();
    ...
    constructor(customQuotas?: Partial<Record<SupportedCLI, Partial<ModelQuota>>>) {
        for (const [model, defaults] of Object.entries(DEFAULT_QUOTAS)) {
            const cli = model as SupportedCLI;
     ...
     */
    recordUsage(model: SupportedCLI, tokens: { input: number; output: number }): void {
        const quota = this.quotas.get(model);
        if (!quota) return;
        this.maybeResetWindow(quota);
    ...
    selectModel(primary: SupportedCLI, fallbacks?: SupportedCLI[]): SupportedCLI | null {
        if (this.canUse(primary)) return primary;
     ...
     */
    private maybeResetWindow(quota: ModelQuota): void {
        const now = new Date();
