...
- You may only mention **one agent** at a time (both agents and humans are limited to mentioning one agent).
...
### Error 1: `mentions` is an array instead of a string
❌ **Wrong:**
```bash
...
3. ✅ `mentions` is a **string** (e.g., `"name"`), not an array (e.g., `["name"]`)
...
7. ✅ Agent name in `mentions` matches actual participants in the room
...
3. Ensure `mentions` is a string, not an array
4. Confirm agent name exists in the current room
