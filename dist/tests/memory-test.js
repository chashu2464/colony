...
const ContextScheduler_js_1 = require("../memory/ContextScheduler.js");
// ── Test Helpers ─────────────────────────────────────────
function createTestMessage(id, content, roomId = 'test-room') {
    return {
        id,
        roomId,
...
function createTestAgentConfig(id, name) {
    return {
        id,
        name,
...
async function runTests() {
    ...
    const stm2 = new ShortTermMemory_js_1.ShortTermMemory();
    const assembler = new ContextAssembler_js_1.ContextAssembler(stm2);
    ...
    const stm3 = new ShortTermMemory_js_1.ShortTermMemory();
    const scheduler = new ContextScheduler_js_1.ContextScheduler(stm3);
    // Set sharing policy
