...
exports.ContextScheduler = void 0;
const Logger_js_1 = require("../utils/Logger.js");
const log = new Logger_js_1.Logger('ContextScheduler');
class ContextScheduler {
     ...
     */
    getSharedMemory(agentId, roomId) {
        ...
        switch (policy.mode) {
            case 'isolated':
                ...
                return allMessages.filter(m => m.sender.id === agentId || m.mentions.includes(agentId));
            case 'shared':
    ...
    async exportSession(roomId) {
        ...
        const keyDecisions = messages.filter(m => this.isKeyDecision(m));
     ...
     */
    async importSession(snapshot, newRoomId) {
     ...
     */
    applySelectivePolicy(agentId, messages, policy) {
        if (!policy.rules || policy.rules.length === 0) {
            return messages; // No rules, default to shared
        }
        // Find rules that apply to this agent
        const applicableRules = policy.rules.filter(rule => rule.to.includes(agentId) || rule.to.includes('*'));
        if (applicableRules.length === 0) {
            // No rules apply, agent sees only their own messages
            return messages.filter(m => m.sender.id === agentId || m.mentions.includes(agentId));
        }
...
exports.ContextScheduler = ContextScheduler;
